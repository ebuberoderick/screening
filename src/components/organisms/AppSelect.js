...
function AppSelect({label,options}) {
    ...
    return (
        <div onMouseLeave={()=> {setShowOption(false),document.getElementById(label === "" ? "-"+figu:label).blur()}} onClick={()=> setShowOption(!showOption)} className='text-[16px] relative rounded-lg'>
            <input value={selected} readOnly id={label === "" ? "-"+figu:label} className='w-full border focus:border-[#025C56] border-[#025C56] placeholder-shown:border-gray-300 p-3 peer outline-none rounded-lg placeholder:text-transparent' placeholder={label} />
